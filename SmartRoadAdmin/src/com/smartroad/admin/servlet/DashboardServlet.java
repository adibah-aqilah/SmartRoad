...
public class DashboardServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            List<HazardReport> all = new HazardReportDAO().getAllReports();
            Set<String> usernames = new HashSet<>();
            int resolved = 0;

            for (HazardReport report : all) {
                if (report.getUsername() != null && !report.getUsername().isBlank()) {
                    usernames.add(report.getUsername().toLowerCase());
                }
            ...
            List<HazardReport> recent = new ArrayList<>(all.subList(0, Math.min(5, all.size())));

            request.setAttribute("totalUsers", usernames.size());
